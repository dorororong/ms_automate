...
텍스트 선택 / 붙여넣기 / 파일 드롭
  ...
  → 왼쪽 원문 / 오른쪽 저장 요약 확인
  → 제목·주요 날짜/시간은 바로 수정, 필요한 카드만 `상세` 열기
...
5. 대부분의 경우 카드의 체크 상태를 확인하고 `등록`을 누릅니다. 제목과 주요 날짜/시간은 카드에서 바로 바꾸고, 세부 내용을 바꿀 때만 카드의 `상세`를 엽니다.
...
`원문 수정`을 눌렀을 때만 가능하고, 제목과 주요 날짜/시간은 카드에서 바로 편집할 수 있습니다. 나머지 카드 상세 편집은 `상세`를 눌렀을 때만 열립니다.
...
제목·주요 날짜·시간은 카드에서 바로 수정하고, 원문 내용 자체가 바뀐 경우에만 다시 넣어 재분석합니다.
