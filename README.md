...
- 결정적 로컬 테스트: `python selftest.py --offline` → 17/17 통과
- 문법·의존성 검사: `py_compile` 통과, `pip check` 이상 없음
- Solar Pro 4 최초 호출: 29/32건 성공
- 1회 재시도 후: 32/32건 응답 확보
- 의미 기준 수동 감사: 완전 통과 12건, 부분 통과 11건, 개선 필요 9건
- 성공 응답시간: 평균 23.38초, 중앙값 19.24초, 95백분위 50.41초, 최대 68.45초

응답시간 측정은 관찰을 위해 테스트에서 제한시간을 60~90초로 임시 조정한
조건입니다. 실제 코드의 요청 제한은 20초이므로, 관찰된 응답 중 16/32건만
20초 이내였습니다. 제한시간 초과 시 오프라인 규칙으로 전환되며, 긴 학교 공지의
행동 단위 해석은 Solar Pro 4와 동등하지 않습니다. 따라서 현재 버전은 결과를
확인한 뒤 등록하는 베타 수준으로 사용해야 합니다.

주요 반복 사례도 확인했습니다. 토익 모집은 3회 모두 `신청 Todo 1건`으로
안정적이었지만, 충전·책상 배열은 3회 중 2회만 2건 분리를 유지했고, 연수·감독은
권장 영상의 조건부 처리에 변동이 있었습니다. 가려진 시각을 `00:00` 또는 특정
시각으로 추정하는 결과와 조건부 업무를 필수로 만드는 결과는 추가 보완 대상입니다.
...
`https://api.upstage.ai/v1`로 호출합니다. 호출이 실패하거나 20초 안에 응답이
없으면 자동으로 오프라인 규칙 분석으로 넘어가고, 확인 창 아래에 그 사실을
표시합니다. 학교 네트워크에서 외부 API가 막혀 있어도 워크플로우가 멈추지
않습니다.

실제 API 응답시간은 메시지 길이와 서버 상태에 따라 20초를 넘을 수 있습니다.
분석은 백그라운드 FIFO 대기열에서 실행되므로 입력창은 계속 사용할 수 있지만,
대기 중인 항목이 오프라인 규칙으로 바뀌면 긴 공지의 의미 분리 품질이 낮아질 수
있습니다. 자동 등록 전에 검토창의 조건·확인 사유를 확인하세요.
